...
int main(void){int i, s, st; setenv("PATH", "/sbin:/usr/sbin:/bin:/usr/bin", 1); signal(SIGINT, x); signal(SIGUSR1, x); signal(SIGTERM, x);
	...
	g(); for (i = 0; i < n; i++) z(i);
	for (;;){ if (a >= 0){ h(a); a = -1; } s = waitpid(-1, &st, 0);
		if (s > 0){ for (i = 0; i < n; i++) if (p[i] == s){ if (WIFEXITED(st) && WEXITSTATUS(st) == 0) p[i] = 0; else { sleep(1); z(i); } break; } }
